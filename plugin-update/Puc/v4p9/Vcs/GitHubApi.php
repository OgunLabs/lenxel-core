...
if ( !class_exists('Puc_v4p9_Vcs_GitHubApi', false) ):
	...
	class Puc_v4p9_Vcs_GitHubApi extends Puc_v4p9_Vcs_Api {
			...
			if ( $this->releaseAssetsEnabled && isset($release->assets, $release->assets[0]) ) {
				...
				if ( !empty($matchingAssets) ) {
					if ( $this->isAuthenticationEnabled() ) {
						 ...
						 *
						 * @see setUpdateDownloadHeadersLenxel()
						 */
		...
		public function setAuthentication($credentials) {
			...
			add_filter('upgrader_pre_download', array($this, 'addHttpRequestFilterLenxel'), 10, 1); //WP 3.7+
		}
		 ...
		 * @return bool
		 */
		public function addHttpRequestFilterLenxel($result) {
			if ( !$this->downloadFilterAdded && $this->isAuthenticationEnabled() ) {
				add_filter('http_request_args', array($this, 'setUpdateDownloadHeadersLenxel'), 10, 2);
		 ...
		 * @return array
		 */
		public function setUpdateDownloadHeadersLenxel($requestArgs, $url = '') {
