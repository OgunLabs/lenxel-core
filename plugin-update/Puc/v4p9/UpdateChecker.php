...
if ( !class_exists('Puc_v4p9_UpdateChecker', false) ):
	...
	abstract class Puc_v4p9_UpdateChecker {
		...
		protected function installHooks() {
			...
			add_filter('site_transient_' . $this->updateTransient, array($this,'injectUpdateLenxel'));

			//Insert translation updates into the update list.
			add_filter('site_transient_' . $this->updateTransient, array($this, 'injectTranslationUpdatesLenxel'));
			...
			if ( $this->directoryName !== '.' ) {
				add_filter('upgrader_source_selection', array($this, 'fixDirectoryNameLenxel'), 10, 3);
			}

			//Allow HTTP requests to the metadata URL even if it's on a local host.
			add_filter('http_request_host_is_external', array($this, 'allowMetadataHostLenxel'), 10, 2);
		...
		public function removeHooks() {
			remove_filter('site_transient_' . $this->updateTransient, array($this,'injectUpdateLenxel'));
			remove_filter('site_transient_' . $this->updateTransient, array($this, 'injectTranslationUpdatesLenxel'));
			remove_action(
				'delete_site_transient_' . $this->updateTransient,
				array($this, 'clearCachedTranslationUpdates')
			);

			remove_filter('upgrader_source_selection', array($this, 'fixDirectoryNameLenxel'), 10);
			remove_filter('http_request_host_is_external', array($this, 'allowMetadataHostLenxel'), 10);
		 ...
		 * @return bool
		 */
		public function allowMetadataHostLenxel($allow, $host) {
		 ...
		 */
		public function injectUpdateLenxel($updates) {
		 ...
		 * @return stdClass
		 */
		public function injectTranslationUpdatesLenxel($updates) {
		 ...
		 */
		public function fixDirectoryNameLenxel($source, $remoteSource, $upgrader) {
