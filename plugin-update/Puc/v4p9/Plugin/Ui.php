	...
	class Puc_v4p9_Plugin_Ui {
		...
		public function onAdminInit() {
			if ( $this->updateChecker->userCanInstallUpdates() ) {
				...
				add_filter('plugin_row_meta', array($this, 'addViewDetailsLinkLenxel'), 10, 3);
				add_filter('plugin_row_meta', array($this, 'addCheckForUpdatesLinkLenxel'), 10, 2);
		...
		public function addViewDetailsLinkLenxel($pluginMeta, $pluginFile, $pluginData = array()) {
			if ( $this->isMyPluginFile($pluginFile) && !isset($pluginData['slug']) ) {
				$linkText = apply_filters($this->updateChecker->getUniqueName('view_details_link'), __('View details','lenxel-core'));
				if ( !empty($linkText) ) {
					...
					$viewDetailsLink = sprintf('<a href="%s" class="thickbox open-plugin-details-modal" aria-label="%s" data-title="%s">%s</a>',
						...
						esc_attr(sprintf(__('More information about %s', 'lenxel-core'), $pluginData['Name'])),
		 ...
		 * @return array
		 */
		public function addCheckForUpdatesLinkLenxel($pluginMeta, $pluginFile) {
			if ( $this->isMyPluginFile($pluginFile) ) {
				...
				$linkText = apply_filters(
					...
					__('Check for updates', 'lenxel-core')
				);
				if ( !empty($linkText) ) {
			...
			if ( $shouldCheck ) {
				...
				wp_redirect(add_query_arg(
					array(
						...
						'puc_slug'                => $this->updateChecker->slug,
						'_wpnonce'                => wp_create_nonce( 'puc_update_check_result' )
					),
					self_admin_url('plugins.php')
				));
				exit;
		...
		public function displayManualCheckResult() {
			if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->updateChecker->slug) && wp_verify_nonce(sanitize_text_field( wp_unslash($_GET['_wpnonce'])), 'puc_update_check_result') ) {
				...
				if ( $status == 'no_update' ) {
					$message = sprintf(_x('The %s plugin is up to date.', 'the plugin title', 'update-checker'), $title);
				} else if ( $status == 'update_available' ) {
					$message = sprintf(_x('A new version of the %s plugin is available.', 'the plugin title', 'update-checker'), $title);
				} else if ( $status === 'error' ) {
					$message = sprintf(_x('Could not determine if updates are available for %s.', 'the plugin title', 'update-checker'), $title);
					$noticeClass = 'error notice-error';

					$details = $this->formatManualCheckErrors(get_site_transient($this->manualCheckErrorTransient));
					delete_site_transient($this->manualCheckErrorTransient);
				} else {
					$message = sprintf(__('Unknown update checker status "%s"', 'lenxel-core'), htmlentities($status));
					$noticeClass = 'error notice-error';
				}
				printf(
		...
		public function removeHooks() {
			...
			remove_filter('plugin_row_meta', array($this, 'addViewDetailsLinkLenxel'), 10);
			remove_filter('plugin_row_meta', array($this, 'addCheckForUpdatesLinkLenxel'), 10);
