...
if(!function_exists('lenxel_post_type_team')){
    function lenxel_post_type_team(){
      $labels = array(
        'name' => __( 'Team', 'lenxel-theme-support' ),
        'singular_name' => __( 'Team', 'lenxel-theme-support' ),
        'add_new' => __( 'Add New Team', 'lenxel-theme-support' ),
        'add_new_item' => __( 'Add New Team', 'lenxel-theme-support' ),
        'edit_item' => __( 'Edit Team', 'lenxel-theme-support' ),
        'new_item' => __( 'New Team', 'lenxel-theme-support' ),
        'view_item' => __( 'View Team', 'lenxel-theme-support' ),
        'search_items' => __( 'Search Teams', 'lenxel-theme-support' ),
        'not_found' => __( 'No Teams found', 'lenxel-theme-support' ),
        'not_found_in_trash' => __( 'No Teams found in Trash', 'lenxel-theme-support' ),
        'parent_item_colon' => __( 'Parent Team:', 'lenxel-theme-support' ),
        'menu_name' => __( 'Teams', 'lenxel-theme-support' ),
      );
  ...
  function lenxelthemesupport_team_socials() {
      add_meta_box(
          ...
          __( 'Socials', 'lenxel-theme-support' ),
      ...
      if ( ($team_socials) && count( $team_socials ) > 0 ) {
          foreach( $team_socials as $social ) {
              if ( isset( $social['icon'] ) || isset( $social['link'] ) ) {
                  printf( '<p><input size="20" type="text" placeholder="Class Icon" name="team_socials[%1$s][icon]" value="%2$s" /><input size="100" type="text" placeholder="Link" name="team_socials[%1$s][link]" value="%3$s" /><a class="button remove">%4$s</a></p>', $c, $social['icon'], $social['link'], __( 'Remove', 'lenxel-theme-support' ) );
                  $c = $c +1;
              }
          }
      }
  ...
  <a class="add-social-item"><?php _e('Add Social','lenxel-theme-support'); ?></a>
  <script>
  ...
  function lenxelthemesupport_team_education() {
    add_meta_box(
        ...
        __( 'Education', 'lenxel-theme-support' ),
      ...
      if ( ($team_educations) && count( $team_educations ) > 0 ) {
          foreach( $team_educations as $education ) {
              if ( isset( $education['title'] ) ) {
                  printf( '<p><input size="120" type="text" placeholder="Title: MBA, Rotterdam School of Management, Erasmus University" name="team_educations[%1$s][title]" value="%2$s" /><a class="button remove">%3$s</a></p>', $c, $education['title'], __( 'Remove', 'lenxel-theme-support') );
                  $c = $c +1;
              }
          }
      }
  ...
  <a class="add-education-item"><?php _e('Add Education','lenxel-theme-support'); ?></a>
  <script>
      jQuery(document).ready(function() {
          var count = <?php echo $c; ?>;
  ...
  function lenxelthemesupport_team_skills() {
    add_meta_box(
        ...
        __( 'Skills', 'lenxel-theme-support' ),
      ...
      if ( ($team_skills) && count( $team_skills ) > 0 ) {
          foreach( $team_skills as $skill ) {
              if ( isset( $skill['label'] ) || isset( $skill['volume'] ) ) {
                  printf( '<p><input size="80" type="text" placeholder="Label" name="team_skills[%1$s][label]" value="%2$s" /><input size="20" type="text" placeholder=" Volume (max 100)" name="team_skills[%1$s][volume]" value="%3$s" /><a class="button remove">%4$s</a></p>', $c, $skill['label'], $skill['volume'], __( 'Remove', 'lenxel-theme-support') );
                  $c = $c +1;
              }
          }
      }
  ...
  <a class="add-skills-item"><?php _e('Add Skills','lenxel-theme-support'); ?></a>
  <script>
      jQuery(document).ready(function() {
          var count = <?php echo $c; ?>;
