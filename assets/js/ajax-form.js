  	...
  	var addonAjaxForm = {
		...
		ajaxChangeUserInfo: function(){
            ...
            $.ajax({
              ...
              url: form_ajax_object.ajaxurl,
              data: {
	            	'action': "lenxel_change_user_info",
		...
		deactivate: function() {
			...
			$('#lenxelConfirmDeactivation').on('click', function(e) {
				e.preventDefault();
				const radioFeedbacks = document.querySelectorAll('input[name="feedback[cause]"]');
    			let feedback = null;

    			radioFeedbacks.forEach(function(radioFeedback) {
					if (radioFeedback.checked) {
						// Get the value of the selected radio button
						feedback = radioFeedback.value;
						return; // Exit the loop if a radio button is checked
					}
    			});
				...
				$.ajax({
					...
					data: {
						...
						'feedback': feedback,
						'comment': $('input[name="feedback[comment]"]').val(),
						'email': $('input[name="feedback[email]"]').val()
					},
					success: function(response) {
						console.log(response);
						// Handle success response (e.g., display message, refresh page, etc.)
						// $('#deactivateModal').fadeOut();
						const closeModalBtnnew = document.querySelector(".btn-close");
						const closeModalnew = function () {
							modal.classList.add("hidden");
							overlay.classList.add("hidden");
						};
						closeModalBtnnew.addEventListener("click", closeModalnew);
			...
			$('#lenxelConfirmDeactivation').prop('disabled',true);
			...
			$('input[name="feedback[cause]"]').on('click', function() {
				$('.betterPlugin').css('display', 'none');
				$('.feedbackOther').css('display', 'none');
				$('#lenxelConfirmDeactivation').prop('disabled',false);
			});
			$('#lenxel-cause-05').click(function(){
				$('.betterPlugin').css('display', 'block');
			});
			$('#lenxel-cause-07').click(function(){
				$('.feedbackOther').css('display', 'block');
			});
		},
		stop_drag_and_drop: function() {
         
					const draggableElements = document.querySelectorAll('.thank-God');
					// Loop through each draggable element
					if(draggableElements){
						draggableElements.forEach(function(element) {
						// Add mousedown event listener
							element.addEventListener('mousedown', function(event) {
								// Check if the clicked element has the class 'not-draggable'
								event.preventDefault();
								event.stopPropagation();
								return;
								
								// Implement your draggable logic here (e.g., setting draggable attribute, handling drag events, etc.)
								// For demonstration, we are simply logging the drag action
