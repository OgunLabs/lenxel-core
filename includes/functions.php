...
{
  $event_handler = "if(typeof trigger_dialog_pro_lenxel === 'function') { trigger_dialog_pro_lenxel(event, '" . esc_attr($title . ' Widget') . "', '" . esc_attr('Use ' . $title . ' widget and dozens more pro features to extend your toolbox and build sites faster and better.') . "'); } event.preventDefault(); event.stopPropagation(); return false;";
  $get_current_name = $title . ( '<div class="upgrade-action" style="position: absolute;z-index: 9990;left: 0;top: 0;padding: 10px 15px 75px 106px;" onmousedown="' . $event_handler . '" onclick="' . $event_handler . '"><span class="eicon-lock" style="position: absolute;top: 10px;right:20px;"></span></div>');
