<?php
/**
 * The template for the panel header area.
 * Override this template by specifying the path where it is stored (templates_path) in your Redux config.
 *
 * @author      Redux Framework
 * @package     ReduxFramework/Templates
 * @version:    4.0.0
 */

$redux_tip_title = esc_html__( 'Developer Mode Enabled', 'lenxel-core' );

if ( $this->parent->args_class->dev_mode_forced ) {
	$redux_is_debug     = false;
	$redux_is_localhost = false;
	$redux_debug_bit    = '';

	if ( Redux_Helpers::is_wp_debug() ) {
		$redux_is_debug  = true;
		$redux_debug_bit = esc_html__( 'WP_DEBUG is enabled', 'lenxel-core' );
	}

	$redux_localhost_bit = '';
	if ( Redux_Helpers::is_local_host() ) {
		$redux_is_localhost  = true;
		$redux_localhost_bit = esc_html__( 'you are working in a localhost environment', 'lenxel-core' );
	}

	$redux_conjunction_bit = '';
	if ( $redux_is_localhost && $redux_is_debug ) {
		$redux_conjunction_bit = ' ' . esc_html__( 'and', 'lenxel-core' ) . ' ';
	}

	$redux_tip_msg = esc_html__( 'This has been automatically enabled because', 'lenxel-core' ) . ' ' . $redux_debug_bit . $conjunction_bit . $redux_localhost_bit . '.';
} else {
	$redux_tip_msg = esc_html__( 'If you are not a developer, your theme/plugin author shipped with developer mode enabled. Contact them directly to fix it.', 'lenxel-core' );
}

?>
<div id="redux-header">
	<?php if ( ! empty( $this->parent->args['display_name'] ) ) { ?>
		<div class="display_header">
			<?php if ( isset( $this->parent->args['dev_mode'] ) && $this->parent->args['dev_mode'] ) { ?>
				<div
					class="redux-dev-mode-notice-container redux-dev-qtip"
					qtip-title="<?php echo esc_attr( $redux_tip_title ); ?>"
					qtip-content="<?php echo esc_attr( $redux_tip_msg ); ?>">
					<span class="redux-dev-mode-notice"><?php esc_html_e( 'Developer Mode Enabled', 'lenxel-core' ); ?></span>
				</div>
			<?php } ?>
			<h2><?php echo wp_kses_post( $this->parent->args['display_name'] ); ?></h2>
			<?php if ( ! empty( $this->parent->args['display_version'] ) ) { ?>
				<span><?php echo wp_kses_post( $this->parent->args['display_version'] ); ?></span>
			<?php } ?>
		</div>
	<?php } ?>
	<div class="clear"></div>
</div>
