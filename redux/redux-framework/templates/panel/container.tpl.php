...
$nonce    = wp_create_nonce( 'redux_ajax_nonce' . $this->parent->args['opt_name_triger'] );
$actionn  = ( 'network' === $this->parent->args['database'] && $this->parent->args['network_admin'] && is_network_admin() ? './edit.php?action=redux_' . $this->parent->args['opt_name_triger'] : './options.php' );
...
?>
	<div
		...
		<form
			...
			data-opt-name="<?php echo esc_attr( $this->parent->args['opt_name_triger'] ); ?>">
			<input
				type="hidden" id="redux-compiler-hook"
				name="<?php echo esc_attr( $this->parent->args['opt_name_triger'] ); ?>[compiler]"
				value=""/>
			<input
				type="hidden" id="currentSection"
				name="<?php echo esc_attr( $this->parent->args['opt_name_triger'] ); ?>[redux-section]"
				value=""/>
			<?php if ( ! empty( $this->parent->options_class->no_panel ) ) { ?>
				<input
					type="hidden"
					name="<?php echo esc_attr( $this->parent->args['opt_name_triger'] ); ?>[redux-no_panel]"
			...
			<input
				...
				name="<?php echo esc_attr( $this->parent->args['opt_name_triger'] ); ?>[last_tab]"
...
<?php } 
		...
		if(function_exists('lenxel_get_start_page')){
			lenxel_get_start_page();
		}
	}
