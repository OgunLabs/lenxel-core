		...
		public function init() {
			...
			if ( isset( $run_hooks ) && true === $run_hooks ) {
				...
				if ( is_admin() && in_array( $pagenow, $this->pagenows, true ) && isset( $_GET['taxonomy'] ) ) {
					...
					// Validate taxonomy against registered taxonomies to prevent arbitrary hook execution.
					$taxonomy = sanitize_text_field( wp_unslash( $_GET['taxonomy'] ) );
					if ( taxonomy_exists( $taxonomy ) ) {
						add_action( $taxonomy . '_edit_form', array( $this, 'add_meta_terms' ), $priority );
						add_action( $taxonomy . '_add_form_fields', array( $this, 'add_meta_terms' ), $priority );
					}
				}
				// phpcs:enable WordPress.Security.NonceVerification
			}
