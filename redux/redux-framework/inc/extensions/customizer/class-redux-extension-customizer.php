...
if ( ! class_exists( 'Redux_Extension_Customizer', false ) ) {
	...
	class Redux_Extension_Customizer extends Redux_Extension_Abstract {
		...
		private function load() {
			...
			add_filter(
				"redux/extension/{$this->parent->args['opt_name_triger']}/customizer",
				array(
					$this,
					'remove_core_customizer_class_lenxel',
				)
			);
			...
			if ( isset( $_POST['wp_customize'] ) && 'on' === $_POST['wp_customize'] && isset( $_POST['customized'] ) && ! empty( $_POST['customized'] ) && ! isset( $_POST['action'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
				add_action(
					"redux/options/{$this->parent->args['opt_name_triger']}/options",
					array(
						$this,
						'override_values',
					),
		 ...
		 * @return string
		 */
		public function remove_core_customizer_class_lenxel( string $path ): string {
			return '';
		}
			...
			if ( isset( $_POST['customized'] ) && ! empty( self::$post_values ) ) { // phpcs:ignore WordPress.Security.NonceVerification
				if ( is_array( self::$post_values ) ) {
					foreach ( self::$post_values as $key => $value ) {
						if ( strpos( $key, $this->parent->args['opt_name_triger'] ) !== false ) {

							$key          = str_replace( $this->parent->args['opt_name_triger'] . '[', '', rtrim( $key, ']' ) );
							$data[ $key ] = $value;
		...
		public function render( $control ) {
			$field_id = str_replace( $this->parent->args['opt_name_triger'] . '-', '', $control->redux_id );
		...
		public function register_customizer_controls( WP_Customize_Manager $wp_customize ) {
			...
			foreach ( $this->parent->sections as $key => $section ) {
				...
				$section['id'] = $this->parent->args['opt_name_triger'] . '-' . $section['id'];
				...
				if ( method_exists( $wp_customize, 'add_panel' ) && ( ! isset( $section['subsection'] ) || ( isset( $section['subsection'] ) && true !== $section['subsection'] ) ) && isset( $this->parent->sections[ ( $key + 1 ) ]['subsection'] ) && $this->parent->sections[ ( $key + 1 ) ]['subsection'] ) {
					$this->add_panel(
						$this->parent->args['opt_name_triger'] . '-' . $section['id'],
						array(
							'priority'    => $section['priority'],
							'capability'  => $section['permissions'],
							'title'       => $section['title'],
							'section'     => $section,
							'opt_name_triger'    => $this->parent->args['opt_name_triger'],
							'description' => '',
						),
						$wp_customize
					);

					$panel = $this->parent->args['opt_name_triger'] . '-' . $section['id'];
				...
				foreach ( $section['fields'] as $skey => $option ) {
					...
					add_action(
						'redux/advanced_customizer/control/render/' . $this->parent->args['opt_name_triger'] . '-' . $option['id'],
						array(
							$this,
							'render',
						)
					...
					$option['id'] = $this->parent->args['opt_name_triger'] . '[' . $option['id'] . ']';
					...
					if ( ! empty( $option['data'] ) && empty( $option['options'] ) ) {
						...
						$option['options'] = $this->parent->wordpress_data->get( $option['data'], $option['args'], $this->parent->args['opt_name_triger'] );
					}
			...
			if ( isset( $_POST['customized'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
				...
				if ( $changed ) {
					...
					if ( $compiler ) {
						...
						// phpcs:ignore WordPress.NamingConventions.ValidHookName
						do_action( "redux/options/{$this->parent->args['opt_name_triger']}/compiler", $this->parent->options, $this->parent->compilerCSS );

						// phpcs:ignore WordPress.NamingConventions.ValidHookName
						do_action( "redux/options/{$this->args['opt_name_triger']}/compiler/advanced", $this->parent );
					}
				}
			}
		}
		...
		public function enqueue() {
			...
			// phpcs:ignore WordPress.NamingConventions.ValidHookName
			do_action( 'redux-enqueue-' . $this->args['opt_name_triger'] );
