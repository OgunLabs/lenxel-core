...
if ( ! class_exists( 'Redux_Extension_Custom_Fonts' ) ) {
	...
	class Redux_Extension_Custom_Fonts extends Redux_Extension_Abstract {
		...
		// Only allow font file types - removed insecure .zip to comply with WordPress security guidelines
		...
		// SVG fonts only - not allowing full SVG uploads for security
		// Note: This is specifically for font files in @font-face, not arbitrary SVG images
				...
				foreach ( $fonts as $section ) {
					if ( 'd' === $section['type'] && ! empty( $section['name'] ) ) {
						if ( 'custom' === $section['name'] ) {
							$section['name'] = esc_html__( 'Custom Fonts', 'lenxel-core' );
						}
		...
		public function ajax() {
			...
			if ( isset( $_POST['type'] ) && 'delete' === $_POST['type'] ) {
				if ( isset( $_POST['section'] ) ) {
					if ( esc_html__( 'Custom Fonts', 'lenxel-core' ) === $_POST['section'] ) {
				...
				try {
					if ( isset( $_POST['section'] ) || isset( $_POST['name'] ) ) {
						...
						if ( true === $ret ) {
							...
							$result = array(
								'type' => 'error',
								'msg'  => esc_html__( 'File system failure. Could not delete temp dir.', 'lenxel-core' ),
							);
						}
					...
					echo wp_json_encode(
						array(
							'type' => 'error',
							'msg'  => esc_html__( 'Unable to delete font file(s).', 'lenxel-core' ),
						)
					);
				}
		...
		public function process_web_font( string $attachment_id, string $mime_type ) {
			...
			if ( empty( $this->selected_file ) ) {
				echo wp_json_encode(
					array(
						'type' => 'error',
						'msg'  => esc_html__( 'Attachment does not exist.', 'lenxel-core' ),
					)
				);
			...
			if ( 'zip' === $subtype ) {
				...
				if ( is_wp_error( $unzipfile ) ) {
					echo wp_json_encode(
						array(
							'type' => 'error',
							'msg'  => $unzipfile->get_error_message() . '<br><br>' . esc_html__( 'Unzipping failed.', 'lenxel-core' ),
						)
					);
					...
					if ( false === $ret ) {
						if ( false === $this->convert_local_font() ) {
							echo wp_json_encode(
								array(
									'type' => 'error',
									'msg'  => esc_html__( 'File permission error. Local file could not be installed.', 'lenxel-core' ) . ' ' . $subtype,
								)
							);
				...
				echo wp_json_encode(
					array(
						'type' => 'error',
						'msg'  => esc_html__( 'File type not recognized.', 'lenxel-core' ) . ' ' . $subtype,
					)
				);
			...
			if ( is_array( $ret ) && ! empty( $ret ) ) {
				$msg = esc_html__( 'Unidentified error.', 'lenxel-core' );
			...
			if ( ! empty( $this->font_folder ) && ! empty( $missing ) ) {
				...
				if ( ! isset( $main ) ) {
					echo wp_json_encode(
						array(
							'type' => 'error',
							'msg'  => esc_html__( 'No valid font file was found.', 'lenxel-core' ),
						)
					);
				...
				if ( is_wp_error( $response ) ) {
					return array(
						'type' => 'error',
						'msg'  => $response->get_error_message() . '<br><br>' . esc_html__( 'Your font could not be converted at this time. Please try again later.', 'lenxel-core' ),
					);
				...
				if ( ! is_wp_error( $zip ) ) {
					...
					if ( in_array( $font_ext, $unsupported, true ) ) {
						return array(
							...
							'msg'  => $zip->get_error_message() . '<br><br>' . sprintf( esc_html__( 'The font converter does not support %s fonts.', 'lenxel-core' ), $font_ext ),
						);
					} else {
						return array(
							'type' => 'error',
							'msg'  => $zip->get_error_message() . '<br><br>' . esc_html__( 'ZIP error. Your font could not be converted at this time. Please try again later.', 'lenxel-core' ),
						);
					}
				}
		...
		public function add_section() {
			if ( ! isset( $this->parent->fontControl ) ) {
				$this->parent->sections[] = array(
					'title'  => esc_html__( 'Font Control', 'lenxel-core' ),
				...
				for ( $i = count( $this->parent->sections ); $i >= 1; $i-- ) {
					if ( isset( $this->parent->sections[ $i ] ) && isset( $this->parent->sections[ $i ]['title'] ) && esc_html__( 'Font Control', 'lenxel-core' ) === $this->parent->sections[ $i ]['title'] ) {
