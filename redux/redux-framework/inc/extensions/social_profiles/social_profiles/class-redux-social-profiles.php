...
if ( ! class_exists( 'Redux_Social_Profiles' ) ) {
	...
	class Redux_Social_Profiles extends Redux_Field {
		...
		public function render() {
			...
			$def_msg = sprintf( esc_html__( 'Go to the %1$s page to add the Redux Social Widget to any active widget area.', 'lenxel-core' ), sprintf( '<a href="%s">' . esc_html__( 'Widgets', 'lenxel-core' ) . '</a> ', admin_url( 'widgets.php' ) ) );
			...
			foreach ( $this->defaults as $key => $social_provider_default ) {
				...
				$label = ( $social_provider_option && array_key_exists( 'label', $social_provider_option ) && $social_provider_option['label'] ) ? $social_provider_option['label'] : __( 'Link URL', 'lenxel-core' );
				...
				esc_html_e( 'Enabled', 'lenxel-core' );
				echo '</div>';
				...
				$reset_text = __( 'Reset', 'lenxel-core' );
				...
				if ( $color_pickers ) {
					...
					$label = apply_filters( 'redux/extensions/social_profiles/' . $this->opt_name . '/color_picker/text', esc_html__( 'Text', 'lenxel-core' ) );
					...
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					$label = apply_filters( 'redux/extensions/social_profiles/' . $this->opt_name . '/color_picker/background', esc_html__( 'Background', 'lenxel-core' ) );
