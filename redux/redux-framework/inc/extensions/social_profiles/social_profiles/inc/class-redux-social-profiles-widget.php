...
if ( ! class_exists( 'Redux_Social_Profiles_Widget' ) ) {
	...
	class Redux_Social_Profiles_Widget {
		...
		public function load_widget() {
			$x = new Lenxel_Extend_WP_Widget_Factory();
			$x->register( 'Redux_Social_Widget_Display', $this->params );
		}
	}

	/**
	 * Class Lenxel_Extend_WP_Widget_Factory
	 */
	// phpcs:ignore Generic.Files.OneClassPerFile
	class Lenxel_Extend_WP_Widget_Factory extends WP_Widget_Factory {
		/**
		 * Register widget.
		 *
	...
	class Redux_Social_Widget_Display extends WP_Widget {
		...
		public function __construct( $params ) {
			...
			$widget_ops = array(
				...
				'description' => __( 'Display social media links', 'lenxel-core' ),
		...
		public function form( $instance ) {
			$defaults = array(
				'title' => esc_html__( 'Social', 'lenxel-core' ),
			);
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>">
					<?php esc_html_e( 'Title', 'lenxel-core' ); ?>
					:
					<input
						class="widefat"
				...
				<label for="redux-social-icons-info">
					...
					printf( esc_html__( 'Control which icons are displayed and their urls on the %1$sssettings page%2$s', 'lenxel-core' ), '<a href="' . esc_url( admin_url( 'admin.php?page=' . esc_attr( $slug ) . '&tab=' . esc_attr( $tab ) ) ) . '">', '</a>' );
					?>
				</label>
			</p>
			<?php
