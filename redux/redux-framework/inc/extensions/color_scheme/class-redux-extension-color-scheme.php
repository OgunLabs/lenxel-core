	...
	class Redux_Extension_Color_Scheme extends Redux_Extension_Abstract {
		...
		public function parse_ajax() {
			if ( isset( $_REQUEST['nonce'] ) && isset( $_REQUEST['opt_name'] ) && wp_verify_nonce( sanitize_key( wp_unslash( $_REQUEST['nonce'] ) ), 'redux_' . sanitize_text_field( wp_unslash( $_REQUEST['opt_name'] ) ) . '_color_schemes' ) ) {
				...
				wp_die( esc_html__( 'Invalid Security Credentials.  Please reload the page and try again.', 'lenxel-core' ) );
			}
		}
		...
		private function import_schemes() {
			...
			if ( is_array( $content ) ) {
				$content = array_map( 'sanitize_text_field', array_filter( $content ) );
			}
				if ( is_null( $content ) ) {
					$result = array(
						'result' => false,
						'data'   => esc_html__( 'Import unsuccessful! Malformed JSON data detected.', 'lenxel-core' ),
					);
				...
				if ( isset( $content['Default']['color_scheme_name'] ) ) {
					...
					if ( true === Redux_Core::$filesystem->execute( 'put_contents', $import_file, $param_array ) ) {
						$result = array(
							...
							'data'   => sprintf( esc_html__( 'Import successful! Click %s to refresh.', 'lenxel-core' ), '<strong>' . esc_html__( 'OK', 'lenxel-core' ) . '</strong>' ),
						);
					} else {
						$result = array(
							'result' => false,
							'data'   => esc_html__( 'Import unsuccessful! File permission error: Could not write import data to server.', 'lenxel-core' ),
						);
					}
				} else {
					$result = array(
						'result' => false,
						'data'   => esc_html__( 'Import unsuccessful! The selected file is not a valid color scheme file.', 'lenxel-core' ),
					);
				}
			} else {
				$result = array(
					'result' => false,
					'data'   => esc_html__( 'Import unsuccessful! No data detected in the import file.', 'lenxel-core' ),
				);
			}
		...
		private function save_scheme( ReduxFramework $redux ) {
			...
			if ( isset( $_REQUEST['scheme_name'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
				...
				if ( isset( $_REQUEST['scheme_data'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
					$scheme_data = wp_kses_post( wp_unslash( $_REQUEST['scheme_data'] ) );

					// Get field ID.
					if ( isset( $_REQUEST['field_id'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
						$scheme_data = rawurldecode( $scheme_data );
						$scheme_data = json_decode( $scheme_data, true );						// Sanitize decoded JSON data
						if ( is_array( $scheme_data ) ) {
							$scheme_data = Redux_Helpers::sanitize_array( $scheme_data );
						}
