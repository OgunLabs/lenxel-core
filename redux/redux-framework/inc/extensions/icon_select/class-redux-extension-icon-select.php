	...
	class Redux_Extension_Icon_Select extends Redux_Extension_Abstract {
		...
		public function get_icons() {
			...
			if ( ! wp_verify_nonce( $nonce, 'redux_icon_nonce' ) ) {
				wp_send_json_error( array( 'error' => esc_html__( 'Error: Invalid nonce verification.', 'lenxel-core' ) ) );
			}
			...
			if ( 'font-awesome' === $icon_set ) {
				// Try to load from uploads directory first (WordPress.org compliant location).
				$upload_dir = wp_upload_dir();
				$redux_file = $upload_dir['basedir'] . '/lenxel-core/redux/font-awesome-6-free.php';
				
				if ( file_exists( $redux_file ) ) {
					require_once $redux_file;
				} else {
					// Fallback to plugin directory for existing installations.
			...
			if ( ! empty( $icon_lists ) ) {
				...
				echo '<div class="redux-error-text">' . esc_html__( 'No data available.', 'lenxel-core' ) . '</div>';
			}
