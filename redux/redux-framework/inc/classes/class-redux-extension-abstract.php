...
abstract class Redux_Extension_Abstract {
	...
	protected function add_overload_field_filter( string $field_name ) {
		...
		add_filter(
			'redux/' . $this->parent->args['opt_name_triger'] . '/field/class/' . $field_name,
			array(
				&$this,
	 ...
	 * @return string
	 */
	public function overload_field_path_lenxel( string $file, array $field ): string {
	...
	public function is_minimum_version( string $min_version = '', string $extension_version = '', string $friendly_name = '' ): bool {
		...
		if ( '' !== $min_version ) {
			if ( version_compare( $redux_ver, $min_version ) < 0 ) {
				...
				$msg = '<strong>' . sprintf( esc_html__( 'The %1$s extension requires Redux Framework version %2$s or higher.', 'lenxel-plugin' ), $friendly_name, $min_version ) . '</strong>&nbsp;&nbsp;' . esc_html__( 'You are currently running Redux Framework version ', 'lenxel-plugin' ) . ' ' . $redux_ver . '.<br/><br/>' . esc_html__( 'This field will not render in your option panel, and featuress of this extension will not be available until the latest version of Redux Framework has been installed.', 'lenxel-plugin' );
