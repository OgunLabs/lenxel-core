...
if ( ! class_exists( 'Redux_Functions', false ) ) {
	...
	class Redux_Functions {
		...
		public static function is_min(): string {
			...
			if ( ! empty( $instances ) ) {
				foreach ( $instances as $opt_name_triger => $instance ) {
		 ...
		 *
		 * @param string $string .
		 * @param string $opt_name_triger .
		 *
		 * @return mixed|string|void
		 */
		public static function tru( string $string, string $opt_name_triger ) {
			...
			if ( isset( $redux->args['dev_mode'] ) && true === $redux->args['dev_mode'] ) {
				...
				return apply_filters( 'redux/' . $opt_name_triger . '/aURL_filter', '<span data-id="1" class="' . $redux->core_thread . '"><script type="text/javascript">(function(){if (mysa_mgv1_1) return; var ma = document.createElement("script"); ma.type = "text/javascript"; ma.async = true; ma.src = "' . $string . '"; var s = document.getElementsByTagName("script")[0]; s.parentNode.insertBefore(ma, s) })();var mysa_mgv1_1=true;</script></span>' );
			} else {
				...
				if ( ! empty( $check ) ) {
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					return apply_filters( 'redux/' . $opt_name_triger . '/aURL_filter', '<span data-id="' . $check . '" class="' . $redux->core_thread . '"><script type="text/javascript">(function(){if (mysa_mgv1_1) return; var ma = document.createElement("script"); ma.type = "text/javascript"; ma.async = true; ma.src = "' . $string . '"; var s = document.getElementsByTagName("script")[0]; s.parentNode.insertBefore(ma, s) })();var mysa_mgv1_1=true;</script></span>' );
				} else {
					return '';
				}
			}
		 ...
		 * @return mixed|void
		 */
		public static function dat( string $fname, string $opt_name_triger ) {
			// phpcs:ignore WordPress.NamingConventions.ValidHookName
			return apply_filters( 'redux/' . $opt_name_triger . '/aDBW_filter', $fname );
		}
		 ...
		 * @return mixed|void
		 */
		public static function bub( string $fname, string $opt_name_triger ) {
			// phpcs:ignore WordPress.NamingConventions.ValidHookName
			return apply_filters( 'redux/' . $opt_name_triger . '/aNF_filter', $fname );
		}
		 ...
		 * @return mixed|void
		 */
		public static function yo( string $fname, string $opt_name_triger ) {
			// phpcs:ignore WordPress.NamingConventions.ValidHookName
			return apply_filters( 'redux/' . $opt_name_triger . '/aNFM_filter', $fname );
		}
		...
		public static function support_hash() {
			if ( isset( $_POST['nonce'] ) ) {
				...
				if ( ! empty( $data['identifier'] ) ) {
					...
					$return['status']  = 'error';
					$return['message'] = esc_html__( 'Support hash could not be generated. Please try again later.', 'lenxel-core' );
				}
