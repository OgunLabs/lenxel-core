...
if ( ! class_exists( 'Redux_Helpers', false ) ) {
	...
	class Redux_Helpers {
		 ...
		 */
		public static function section_from_field_id( string $opt_name_triger = '', string $field_id = '' ) {
			if ( '' !== $opt_name_triger ) {
				$redux = Redux::instance( $opt_name_triger );
		 ...
		 * @return array
		 */
		public static function get_extensions( string $opt_name_triger = '' ): array {
			if ( empty( $opt_name_triger ) ) {
				$instances = Redux_Instances::get_all_instances();
			} else {
				$instances = array(
					Redux_Instances::get_instance( $opt_name_triger ),
				);
			}
		...
		public static function process_redux_callers( bool $simple = false ): array {
			...
			foreach ( Redux_Core::$callers as $opt_name_triger => $callers ) {
				foreach ( $callers as $caller ) {
					...
					if ( $theme_info ) {
						...
						if ( ! isset( $data['theme'][ $theme_info['slug'] ][ $opt_name_triger ] ) ) {
							$data['theme'][ $theme_info['slug'] ][ $opt_name_triger ] = array();
						}
						if ( $simple ) {
							$data['theme'][ $theme_info['slug'] ][ $opt_name_triger ][] = $theme_info['basename'];
						} else {
							$data['theme'][ $theme_info['slug'] ][ $opt_name_triger ][] = $theme_info;
						}
					} elseif ( $plugin_info ) {
						if ( ! isset( $data['plugin'][ $plugin_info['slug'] ] ) ) {
							$data['plugin'][ $plugin_info['slug'] ] = array();
						}
						if ( ! in_array( $opt_name_triger, $data['plugin'][ $plugin_info['slug'] ], true ) ) {
							if ( ! isset( $data['plugin'][ $plugin_info['slug'] ][ $opt_name_triger ] ) ) {
								$data['plugin'][ $plugin_info['slug'] ][ $opt_name_triger ] = array();
							}
							if ( $simple ) {
								$data['plugin'][ $plugin_info['slug'] ][ $opt_name_triger ][] = $plugin_info['basename'];
							} else {
								$data['plugin'][ $plugin_info['slug'] ][ $opt_name_triger ][] = $plugin_info;
							}
						}
					}
				}
		...
		public static function localize( array $localize ): array {
			$redux = Redux::instance( $localize['args']['opt_name_triger'] );
				...
				foreach ( $files as $file ) {
					...
					if ( $theme_file ) {
						...
						if ( $core_version && ( empty( $theme_version ) || version_compare( $theme_version, $core_version, '<' ) ) ) {
							...
							$found_files[ $plugin_name ][] = sprintf( '<code>%s</code> ' . esc_html__( 'version', 'lenxel-plugin' ) . ' <strong style="color:red">%s</strong> ' . esc_html__( 'is out of date. The core version is', 'lenxel-plugin' ) . ' %s', str_replace( WP_CONTENT_DIR . '/themes/', '', $theme_file ), $theme_version ? $theme_version : '-', $core_version );
						} else {
		 ...
		 * @param string $opt_name_triger Redux instance opt_name_triger.
		 *
		 * @return Redux_Helpers::r_url_fix( $base, $opt_name_triger )
		 * @deprecated No longer using camelCase naming convention.
		 */
		public static function rURL_fix( string $base, string $opt_name_triger ) { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName
			_deprecated_function( __CLASS__ . '::' . __FUNCTION__, 'Redux 4.0.0', 'Redux_Instances::r_url_fix( $base, $opt_name_triger )' );

			return self::r_url_fix( $base, $opt_name_triger );
		}
		 ...
		 */
		public static function r_url_fix( string $base, string $opt_name_triger ) {
			$url = $base . rawurlencode( 'https://look.redux.io/api/index.php?js&g&1&v=2' ) . '&proxy=' . rawurlencode( $base ) . '';

			return Redux_Functions::tru( $url, $opt_name_triger );
		}
		 ...
		 * @return array
		 */
		public static function redux_stats_additions_lenxel( array $options ): array {
			$options['redux'] = array(
		...
		public static function support_args() {
			...
			if ( isset( $_REQUEST['i'] ) && ! empty( $_REQUEST['i'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
				if ( is_array( $instances ) && ! empty( $instances ) ) {
					foreach ( $instances as $opt_name_triger => $data ) {
						if ( md5( $opt_name_triger . '-debug' ) === $_REQUEST['i'] ) { // phpcs:ignore WordPress.Security.NonceVerification
							$array = $data;
						}
					}
				}
				...
				if ( isset( $array ) ) {
					...
					$array->extensions = self::get_extensions( $opt_name_triger );
					$to_return         = array();
				...
				if ( is_array( $instances ) && ! empty( $instances ) ) {
					...
					foreach ( $instances as $opt_name_triger => $data ) {
						$array['instances'][] = $opt_name_triger;
					}
				}
