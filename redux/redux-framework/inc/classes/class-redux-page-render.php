...
if ( ! class_exists( 'Redux_Page_Render', false ) ) {
	...
	class Redux_Page_Render extends Redux_Class {
		...
		public function load_page() {
			...
			add_filter( 'admin_footer_text', array( $this, 'admin_footer_text_lenxel' ) );
			...
			if ( true === $this->show_hints ) {
				...
				$dismiss = 'true';
				$s       = esc_html__( 'Enable', 'lenxel-theme-support' );

				// Values for disabling hints.
				if ( 'true' === $hint_status ) {
					$dismiss = 'false';
					$s       = esc_html__( 'Disable', 'lenxel-theme-support' );
				}
				...
				$event = esc_html__( 'moving the mouse over', 'lenxel-theme-support' );
				if ( 'click' === $core->args['hints']['tip_effect']['show']['event'] ) {
					$event = esc_html__( 'clicking', 'lenxel-theme-support' );
				}

				// Construct message.
				// translators: %1$s: Mousr action.  %2$s: Hint status.
				$msg = sprintf( esc_html__( 'Hints are tooltips that popup when %1$s the hint icon, offering addition information about the field in which they appear.  They can be %2$s by using the link below.', 'lenxel-theme-support' ), $event, Redux_Core::strtolower( $s ) ) . '<br/><br/>' . $url;

				// Construct hint tab.
				$tab = array(
					'id'      => 'redux-hint-tab',
					'title'   => esc_html__( 'Hints', 'lenxel-theme-support' ),
					'content' => '<p>' . $msg . '</p>',
				);
			...
			if ( '' !== $core->args['help_sidebar'] ) {
				...
				// about hints.
				if ( true === $this->show_hints ) {
					$screen->set_help_sidebar( '<p><strong>Redux Framework</strong><br/><br/>' . esc_html__( 'Hint Tooltip Preferences', 'lenxel-theme-support' ) . '</p>' );
				}
			}

			/**
			 * Action 'redux/page/{opt_name_triger}/load'
			 *
			 * @param object $screen WP_Screen
			 */

			// phpcs:ignore WordPress.NamingConventions.ValidHookName
			do_action( "redux/page/{$core->args['opt_name_triger']}/load", $screen );
		}
		...
		public function section_desc( array $section ) {
			...
			$id = str_replace( $core->args['opt_name_triger'], '', $id );
		...
		public function field_input( array $field, $v = null ) {
			...
			if ( isset( $field['callback'] ) && ( is_callable( $field['callback'] ) || ( is_string( $field['callback'] ) && function_exists( $field['callback'] ) ) ) ) {
				...
				/**
				 * Action 'redux/field/{opt_name_triger}/{field.type}/callback/before'
				 *
				...
				do_action_ref_array(
				// phpcs:ignore WordPress.NamingConventions.ValidHookName
					"redux/field/{$core->args['opt_name_triger']}/{$field['type']}/callback/before",
					array(
						&$field,
						&$value,
					)
				...
				/**
				 * Action 'redux/field/{opt_name_triger}/callback/before'
				 *
				 * @param array  $field field data
				 * @param string $value field.id
				 */
				...
				do_action_ref_array(
				// phpcs:ignore WordPress.NamingConventions.ValidHookName
					"redux/field/{$core->args['opt_name_triger']}/callback/before",
					array(
						&$field,
						&$value,
					)
				...
				/**
				 * Action 'redux/field/{opt_name_triger}/{field.type}/callback/after'
				 *
				 * @param array  $field field data
				 * @param string $value field.id
				 */
				...
				do_action_ref_array(
				// phpcs:ignore WordPress.NamingConventions.ValidHookName
					"redux/field/{$core->args['opt_name_triger']}/{$field['type']}/callback/after",
					array(
						&$field,
						&$value,
					)
				...
				/**
				 * Action 'redux/field/{opt_name_triger}/callback/after'
				 *
				 * @param array  $field field data
				 * @param string $value field.id
				 */
				...
				do_action_ref_array(
				// phpcs:ignore WordPress.NamingConventions.ValidHookName
					"redux/field/{$core->args['opt_name_triger']}/callback/after",
					array(
						&$field,
						&$value,
					)
			...
			if ( isset( $field['type'] ) ) {
				...
				/**
				 * Filter 'redux/{opt_name_triger}/field/class/{field.type}'
				 *
				...
				// phpcs:ignore WordPress.NamingConventions.ValidHookName
				$class_file = apply_filters( "redux/{$core->args['opt_name_triger']}/field/class/{$field['type']}", $filter_path, $field );
				...
				if ( $class_file ) {
					...
					if ( ! class_exists( $field_class ) ) {
						if ( file_exists( $class_file ) ) {
							...
							echo sprintf( esc_html__( 'Field %1$s could not be displayed. Field type %2$s was not found.', 'lenxel-theme-support' ), '<code>' . esc_attr( $field['id'] ) . '</code>', '<code>' . esc_attr( $field['type'] ) . '</code>' );
						}
					}
				}
				...
				if ( class_exists( $field_class ) ) {
					...
					/**
					 * Action 'redux/field/{opt_name_triger}/{field.type}/render/before'
					 *
					 * @param array  $field field data
					 * @param string $value field id
					 */
					...
					do_action_ref_array(
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
						"redux/field/{$core->args['opt_name_triger']}/{$field['type']}/render/before",
						array(
							&$field,
							&$value,
						)
					...
					/**
					 * Action 'redux/field/{$this->args['opt_name_triger']}/render/before'
					 *
					 * @param array  $field field data
					 * @param string $value field id
					 */
					...
					do_action_ref_array(
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
						"redux/field/{$core->args['opt_name_triger']}/render/before",
						array(
							&$field,
							&$value,
						)
					...
					/**
					 * Filter 'redux/field/{opt_name_triger}'
					 *
					...
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					$_render = apply_filters( "redux/field/{$core->args['opt_name_triger']}", ob_get_contents(), $field );

					/**
					 * Filter 'redux/field/{opt_name_triger}/{field.type}/render/after'
					 *
					 * @param       string        rendered field markup
					 * @param array $field field data
					 */
					...
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					$_render = apply_filters( "redux/field/{$core->args['opt_name_triger']}/{$field['type']}/render/after", $_render, $field );

					/**
					 * Filter 'redux/field/{opt_name_triger}/render/after'
					 *
					 * @param       string        rendered field markup
					 * @param array $field field data
					 */
					...
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					$_render = apply_filters( "redux/field/{$core->args['opt_name_triger']}/render/after", $_render, $field );
					...
					/**
					 * Action 'redux/field/{opt_name_triger}/{field.type}/fieldset/before/{opt_name_triger}'
					 *
					 * @param array  $field field data
					 * @param string $value field id
					 */
					...
					do_action_ref_array(
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
						"redux/field/{$core->args['opt_name_triger']}/{$field['type']}/fieldset/before/{$core->args['opt_name_triger']}",
						array(
							&$field,
							&$value,
						)
					...
					/**
					 * Action 'redux/field/{opt_name_triger}/fieldset/before/{opt_name_triger}'
					 *
					 * @param array  $field field data
					 * @param string $value field id
					 */
					...
					do_action_ref_array(
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
						"redux/field/{$core->args['opt_name_triger']}/fieldset/before/{$core->args['opt_name_triger']}",
						array(
							&$field,
							&$value,
						)
					...
					echo '<fieldset id="' . esc_attr( $core->args['opt_name_triger'] . '-' . $field['id'] ) . '" class="' . esc_attr( $hidden . esc_attr( $disabled ) . 'redux-field-container redux-field redux-field-init redux-container-' . $field['type'] . ' ' . $class_string ) . '" data-id="' . esc_attr( $field['id'] ) . '" data-type="' . esc_attr( $field['type'] ) . '">';
					...
					/**
					 * Action 'redux/field/{opt_name_triger}/{field.type}/fieldset/after/{opt_name_triger}'
					 *
					 * @param array  $field field data
					 * @param string $value field id
					 */
					...
					do_action_ref_array(
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
						"redux/field/{$core->args['opt_name_triger']}/{$field['type']}/fieldset/after/{$core->args['opt_name_triger']}",
						array(
							&$field,
							&$value,
						)
					...
					/**
					 * Action 'redux/field/{opt_name_triger}/fieldset/after/{opt_name_triger}'
					 *
					 * @param array  $field field data
					 * @param string $value field id
					 */
					...
					do_action_ref_array(
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
						"redux/field/{$core->args['opt_name_triger']}/fieldset/after/{$core->args['opt_name_triger']}",
						array(
							&$field,
							&$value,
						)
		...
		public function admin_head() {
			...
			/**
			 * Action 'redux/page/{opt_name_triger}/header'
			 *
			 * @param  object $this ReduxFramework
			 */

			// phpcs:ignore WordPress.NamingConventions.ValidHookName
			do_action( "redux/page/{$core->args['opt_name_triger']}/header", $core );
		}
		 ...
		 */
		public function admin_footer_text_lenxel(): string {
			$core = $this->core();
		...
		private function get_default_output_string( array $field ): string {
			...
			if ( ! empty( $default_output ) ) {
				$default_output = esc_html__( 'Default', 'lenxel-theme-support' ) . ': ' . substr( $default_output, 0, - 2 );
			}
