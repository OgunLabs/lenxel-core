		...
		public function __construct( $parent, array $args ) {
			...
			$default = array(
				...
				'font_weights'                     => array(
					array(
						'id'   => '400',
						'name' => __( 'Regular 400', 'lenxel-plugin' ),
					),
					array(
						'id'   => '400italic',
						'name' => __( 'Regular 400 Italic', 'lenxel-plugin' ),
					),
					array(
						'id'   => '700',
						'name' => __( 'Bold 700', 'lenxel-plugin' ),
					),
					array(
						'id'   => '700italic',
						'name' => __( 'Bold 700 Italic', 'lenxel-plugin' ),
					),
				),
		...
		private function no_errors_please( array $args ): array {
			...
			if ( empty( $args['footer_credit'] ) ) {
				...
				/* translators: 1: Redux, 2: Link to plugin review */
					__( 'Enjoyed %1$s? Please leave us a %2$s rating. We really appreciate your support!', 'lenxel-plugin' ),
					'<strong>' . __( 'Redux', 'lenxel-plugin' ) . '</strong>',
			...
			if ( empty( $args['menu_title'] ) ) {
				$args['menu_title'] = esc_html__( 'Options', 'lenxel-plugin' );
			}

			if ( empty( $args['page_title'] ) ) {
				$args['page_title'] = esc_html__( 'Options', 'lenxel-plugin' );
			}
						...
						if ( is_array( $arr ) && ! empty( $arr ) ) {
							foreach ( $arr as $x => $y ) {
								if ( strpos( Redux_Core::strtolower( $y ), 'redux' ) !== false ) {
									$msg = '<strong>' . esc_html__( 'Redux Framework Notice', 'lenxel-plugin' ) . ' </strong>' .
										esc_html__( 'There are references to the Redux Framework support site in your config\'s ', 'lenxel-plugin' ) .
										'<code>admin_bar_links</code> ' . esc_html__( 'argument.  This is sample data.  Please change or remove this data before shipping your product.', 'lenxel-plugin' );
						...
						if ( is_array( $arr ) && ! empty( $arr ) ) {
							foreach ( $arr as $x => $y ) {
								if ( strpos( Redux_Core::strtolower( $y ), 'redux' ) !== false ) {
									$msg = '<strong>' . esc_html__( 'Redux Framework Notice:', 'lenxel-plugin' ) . '</strong>' .
										esc_html__( 'There are references to the Redux Framework support site in your config\'s', 'lenxel-plugin' ) .
										' <code>share_icons</code> ' . esc_html__( 'argument.  This is sample data.  Please change or remove this data before shipping your product.', 'lenxel-plugin' );
