		...
		public static function set_section( string $opt_name_triger = '', ?array $section = array(), bool $replace = false ) {
			...
			if ( ! empty( $opt_name_triger ) && is_array( $section ) && ! empty( $section ) ) {
				if ( ! isset( $section['id'] ) && ! isset( $section['title'] ) ) {
					self::$errors[ $opt_name_triger ]['section']['missing_title'] = esc_html__( 'Unable to create a section due to missing id and title.', 'lenxel-core' );
				...
				self::$errors[ $opt_name_triger ]['section']['empty'] = esc_html__( 'Unable to create a section due an empty section array or the section variable passed was not an array.', 'lenxel-core' );
			}
		}
