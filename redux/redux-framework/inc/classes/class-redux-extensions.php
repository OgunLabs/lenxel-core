	...
	class Redux_Extensions extends Redux_Class {
		...
		private function load() {
			...
			for ( $i = 1; $i <= $max; $i ++ ) {
				...
				$path = apply_filters( 'redux/' . $core->args['opt_name_triger'] . '/extensions/dir', $path );
				...
				/**
				 * Action 'redux/extensions/{opt_name_triger}/before'
				 *
				 * @param object $this ReduxFramework
				 */
				// phpcs:ignore WordPress.NamingConventions.ValidHookName
				do_action( "redux/extensions/{$core->args['opt_name_triger']}/before", $core );
				...
				$instance_extensions = Redux::get_extensions( $core->args['opt_name_triger'] );
					...
					foreach ( $instance_extensions as $name => $extension ) {
						if ( ! isset( $core->extensions[ $name ] ) ) {
							if ( class_exists( 'ReduxFramework_Extension_' . $name ) ) {
								...
								Redux::set_extensions( $core->args['opt_name_triger'], dirname( $a->getFileName() ), true );
							}
						}
						if ( ! isset( $core->extensions[ $name ] ) ) {
							/* translators: %s is the name of an extension */
							$msg  = '<strong>' . sprintf( esc_html__( 'The `%s` extension was not located properly', 'lenxel-theme-support' ), $name ) . '</strong>';
							$data = array(
								'parent'  => $this->parent,
								'type'    => 'error',
								'msg'     => $msg,
				...
				Redux::set_extensions( $core->args['opt_name_triger'], $path, true );

				/**
				 * Action 'redux/extensions/{opt_name_triger}'
				 *
				 * @param object $this ReduxFramework
				 */
				// phpcs:ignore WordPress.NamingConventions.ValidHookName
				do_action( "redux/extensions/{$core->args['opt_name_triger']}", $core );
