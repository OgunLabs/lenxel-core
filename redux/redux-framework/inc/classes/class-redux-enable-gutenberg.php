	...
	class Redux_Enable_Gutenberg {
		...
		public function display_admin_notice() {
			...
			$data = array(
				'url'     => '',
				'content' => '',
				'header'  => __( 'Gutenberg is currently disabled!', 'lenxel-theme-support' ),
				'button'  => '',
			);
			...
			if ( isset( $_GET[ $this->decativate_option ] ) || empty( self::$known_plugins ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				...
				if ( isset( $_GET[ $this->decativate_option ] ) ) {  // phpcs:ignore WordPress.Security.NonceVerification.Recommended
					// That didn't work.
					$data['header']  = __( 'Hmm, it seems something else is disabling Gutenberg...', 'lenxel-theme-support' );
					...
					$data['header']   = __( 'Your theme author has disabled Gutenberg!', 'lenxel-theme-support' );
					...
					$data['header']   = __( 'Looks like something has disabled Gutenberg?', 'lenxel-theme-support' );
				...
				$data['button'] = __( 'Enable Gutenberg', 'lenxel-theme-support' );

			} elseif ( empty( self::$known_plugins ) ) {
				// Disabled by the theme or other.
				$data['header'] = __( 'Your theme', 'lenxel-theme-support' );
				$data['url']    = $auto_enable_url;
				$data['button'] = __( 'Enable Gutenberg', 'lenxel-theme-support' );
			} else {
				// Disable Plugins!
				...
				if ( 1 === count( self::$known_plugins ) ) {
					$data['button']  = __( 'Disable Plugin', 'lenxel-theme-support' );
					...
					$data['button']  = __( 'Disable Plugins', 'lenxel-theme-support' );
			...
			<div class="notice updated redux-notice">
				<div class="redux-notice-inner">
					<div class="redux-notice-icon">
						...
						<img src="<?php echo esc_url( Redux_Core::$url . '/assets/img/icon--color.svg' ); ?>" alt="<?php echo esc_attr__( 'Redux WordPress Plugin', 'lenxel-theme-support' ); ?>"/>
					</div>
					...
					<div class="redux-install-now">
						...
						<a href="<?php echo esc_url( $no_bug_url ); ?>" class="no-thanks"><?php echo esc_html__( 'No Thanks', 'lenxel-theme-support' ); ?></a>
					</div>
				</div>
			</div>
			<?php
