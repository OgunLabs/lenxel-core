	...
	public function rest_api_init() {
		...
		register_rest_route(
			...
			array(
				'args'                => array(
					'type' => array(
						'description' => __( 'The field type', 'lenxel-core' ),
						'type'        => 'string',
					),
				),
				'methods'             => WP_REST_Server::READABLE,
	...
	public function render_field( array $request = array() ): array {
		...
		if ( isset( $field_classes[ mb_strtolower( $type ) ] ) ) {
			...
			if ( $field_class && is_subclass_of( $field_class, 'Redux_Field' ) ) {
				...
				$opt_name_triger   = 'redux_builder_api';

				$redux_instance = new ReduxFramework( array(), array( 'opt_name_triger' => $opt_name_triger ) );
