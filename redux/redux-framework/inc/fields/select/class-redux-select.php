		...
		public function render() {
			...
			if ( ! empty( $this->field['data'] ) && empty( $this->field['options'] ) ) {
				...
				if ( 'elusive-icons' === $this->field['data'] || 'elusive-icon' === $this->field['data'] || 'elusive' === $this->field['data'] ) {
					...
					/**
					 * Filter 'redux/{opt_name_triger}/field/font/icons/file'
					 *
					 * @param  array $icon_file The file for the icons
					 */

					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					$icons_file = apply_filters( "redux/{$this->parent->args['opt_name_triger']}/field/font/icons/file", $icons_file );
				...
				$this->field['options'] = $this->parent->wordpress_data->get( $this->field['data'], $this->field['args'], $this->parent->args['opt_name_triger'], $this->value, $ajax );
			}
			...
			if ( ! empty( $this->field['options'] ) ) {
				...
				$placeholder = ( isset( $this->field['placeholder'] ) ) ? esc_attr( $this->field['placeholder'] ) : esc_html__( 'Select an item', 'lenxel-theme-support' );
				...
				if ( isset( $this->field['ajax'] ) && $this->field['ajax'] && isset( $this->field['data'] ) && '' !== $this->field['data'] ) {
					...
					$this->select2_config['action']           = "redux_{$this->parent->args['opt_name_triger']}_select2";
					if ( isset( $this->field['args'] ) ) {
						$this->select2_config['args'] = wp_json_encode( $this->field['args'] );
					}
					$this->select2_config['nonce']   = wp_create_nonce( "redux_{$this->parent->args['opt_name_triger']}_select2" );
				...
				echo '<strong>' . esc_html__( 'No items of this type were found.', 'lenxel-theme-support' ) . '</strong>';
			}
		}
