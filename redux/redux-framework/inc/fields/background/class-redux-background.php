		...
		public function render() {
			...
			if ( $this->field['background-image'] ) {
				if ( true === $this->field['background-repeat'] ) {
					$array = array(
						'no-repeat' => esc_html__( 'No Repeat', 'lenxel-plugin' ),
						'repeat'    => esc_html__( 'Repeat All', 'lenxel-plugin' ),
						'repeat-x'  => esc_html__( 'Repeat Horizontally', 'lenxel-plugin' ),
						'repeat-y'  => esc_html__( 'Repeat Vertically', 'lenxel-plugin' ),
						'inherit'   => esc_html__( 'Inherit', 'lenxel-plugin' ),
					);

					echo '<select id="' . esc_attr( $this->field['id'] ) . '-repeat-select" data-placeholder="' . esc_html__( 'Background Repeat', 'lenxel-plugin' ) . '" name="' . esc_attr( $this->field['name'] . $this->field['name_suffix'] ) . '[background-repeat]" class="redux-select-item redux-background-input redux-background-repeat ' . esc_attr( $this->field['class'] ) . '"' . esc_attr( $select2_data ) . '>';
					echo '<option></option>';
				...
				if ( true === $this->field['background-clip'] ) {
					$array = array(
						'inherit'     => esc_html__( 'Inherit', 'lenxel-plugin' ),
						'border-box'  => esc_html__( 'Border Box', 'lenxel-plugin' ),
						'content-box' => esc_html__( 'Content Box', 'lenxel-plugin' ),
						'padding-box' => esc_html__( 'Padding Box', 'lenxel-plugin' ),
					);

					echo '<select id="' . esc_attr( $this->field['id'] ) . '-clip-select" data-placeholder="' . esc_html__( 'Background Clip', 'lenxel-plugin' ) . '" name="' . esc_attr( $this->field['name'] . $this->field['name_suffix'] ) . '[background-clip]" class="redux-select-item redux-background-input redux-background-clip ' . esc_attr( $this->field['class'] ) . '"' . esc_attr( $select2_data ) . '>';
					echo '<option></option>';
				...
				if ( true === $this->field['background-origin'] ) {
					...
					echo '<select id="' . esc_attr( $this->field['id'] ) . '-origin-select" data-placeholder="' . esc_html__( 'Background Origin', 'lenxel-plugin' ) . '" name="' . esc_attr( $this->field['name'] . $this->field['name_suffix'] ) . '[background-origin]" class="redux-select-item redux-background-input redux-background-origin ' . esc_attr( $this->field['class'] ) . '"' . esc_attr( $select2_data ) . '>';
					echo '<option></option>';
				...
				if ( true === $this->field['background-size'] ) {
					$array = array(
						'inherit' => esc_html__( 'Inherit', 'lenxel-plugin' ),
						'cover'   => esc_html__( 'Cover', 'lenxel-plugin' ),
						'contain' => esc_html__( 'Contain', 'lenxel-plugin' ),
					);

					echo '<select id="' . esc_attr( $this->field['id'] ) . '-size-select" data-placeholder="' . esc_html__( 'Background Size', 'lenxel-plugin' ) . '" name="' . esc_attr( $this->field['name'] . $this->field['name_suffix'] ) . '[background-size]" class="redux-select-item redux-background-input redux-background-size ' . esc_attr( $this->field['class'] ) . '"' . esc_attr( $select2_data ) . '>';
					echo '<option></option>';
				...
				if ( true === $this->field['background-attachment'] ) {
					$array = array(
						'fixed'   => esc_html__( 'Fixed', 'lenxel-plugin' ),
						'scroll'  => esc_html__( 'Scroll', 'lenxel-plugin' ),
						'inherit' => esc_html__( 'Inherit', 'lenxel-plugin' ),
					);

					echo '<select id="' . esc_attr( $this->field['id'] ) . '-attachment-select" data-placeholder="' . esc_html__( 'Background Attachment', 'lenxel-plugin' ) . '" name="' . esc_attr( $this->field['name'] . $this->field['name_suffix'] ) . '[background-attachment]" class="redux-select-item redux-background-input redux-background-attachment ' . esc_attr( $this->field['class'] ) . '"' . esc_attr( $select2_data ) . '>';
					echo '<option></option>';
				...
				if ( true === $this->field['background-position'] ) {
					$array = array(
						'left top'      => esc_html__( 'Left Top', 'lenxel-plugin' ),
						'left center'   => esc_html__( 'Left center', 'lenxel-plugin' ),
						'left bottom'   => esc_html__( 'Left Bottom', 'lenxel-plugin' ),
						'center top'    => esc_html__( 'Center Top', 'lenxel-plugin' ),
						'center center' => esc_html__( 'Center Center', 'lenxel-plugin' ),
						'center bottom' => esc_html__( 'Center Bottom', 'lenxel-plugin' ),
						'right top'     => esc_html__( 'Right Top', 'lenxel-plugin' ),
						'right center'  => esc_html__( 'Right center', 'lenxel-plugin' ),
						'right bottom'  => esc_html__( 'Right Bottom', 'lenxel-plugin' ),
					);

					echo '<select id="' . esc_attr( $this->field['id'] ) . '-position-select" data-placeholder="' . esc_html__( 'Background Position', 'lenxel-plugin' ) . '" name="' . esc_attr( $this->field['name'] . $this->field['name_suffix'] ) . '[background-position]" class="redux-select-item redux-background-input redux-background-position ' . esc_attr( $this->field['class'] ) . '"' . esc_attr( $select2_data ) . '>';
					echo '<option></option>';
			...
			if ( $this->field['background-image'] ) {
				...
				$placeholder = $this->field['placeholder'] ?? esc_html__( 'No media selected', 'lenxel-plugin' );
				...
				echo '<span class="button redux-background-upload" id="' . esc_attr( $this->field['id'] ) . '-media">' . esc_html__( 'Upload', 'lenxel-plugin' ) . '</span>';
				...
				echo '<span class="button removeCSS redux-remove-background' . esc_attr( $hide ) . '" id="reset_' . esc_attr( $this->field['id'] ) . '" rel="' . esc_attr( $this->field['id'] ) . '">' . esc_html__( 'Remove', 'lenxel-plugin' ) . '</span>';
