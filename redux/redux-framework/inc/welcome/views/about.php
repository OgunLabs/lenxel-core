		...
		<div class="<?php echo ! empty( $redux_sys_info ) ? 'col' : ''; ?>">
			...
			<h3>
				...
				if ( ! empty( $redux_sys_info ) ) {
					...
					printf( esc_html__( 'Maybe not! If you want to see what Redux is all about, click here to %1$sActivate Demo Mode%2$s.', 'lenxel-core' ), '<a href="' . esc_url( admin_url( add_query_arg( $redux_query_args, 'options-general.php' ) ) ) . '">', '</a>' );
				}
						...
						<div class="redux-product">
							<h2 class="name"><?php echo esc_html( $redux_my_theme->get( 'Name' ) ); ?>
								<?php if ( ! empty( $redux_my_theme->get( 'Version' ) ) ) { ?>
									<span class="version"><?php echo esc_html__( 'Version:', 'lenxel-core' ); ?>&nbsp;<?php echo esc_html( $redux_my_theme->get( 'Version' ) ); ?></span>
								<?php } ?>
							</h2>
							<p class="author">
							...
							<div class="redux-product">
								<h2 class="name">
									...
									<?php if ( ! empty( $redux_plugin_data['Version'] ) ) { ?>
										<span class="version"><?php echo esc_html__( 'Version', 'lenxel-core' ); ?>&nbsp;<?php echo esc_html( $redux_plugin_data['Version'] ); ?></span>
									<?php } ?>
								</h2>
								<p class="author">
									<?php
