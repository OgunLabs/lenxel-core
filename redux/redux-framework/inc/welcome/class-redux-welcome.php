...
if ( ! class_exists( 'Redux_Welcome', false ) ) {
	...
	class Redux_Welcome {
		...
		public function do_redirect() {
			if ( ! defined( 'WP_CLI' ) ) {
				wp_safe_redirect( esc_url( admin_url( add_query_arg( array( 'page' => 'lenxel-plugin' ), 'tools.php' ) ) ) );
				exit();
			}
		}
		...
		public function change_wp_footer_lenxel() {
			echo esc_html__( 'If you like', 'lenxel-plugin' ) . ' <strong>Redux</strong> ' . esc_html__( 'please leave us a', 'lenxel-plugin' ) . ' <a href="https://wordpress.org/support/view/plugin-reviews/redux-framework?filter=5#postform" target="_blank" class="redux-rating-link" data-rated="Thanks :)">&#9733;&#9733;&#9733;&#9733;&#9733;</a> ' . esc_html__( 'rating. A huge thank you in advance!', 'lenxel-plugin' );
		}
		...
		public function admin_menus() {
			...
			// About Page.
			$page( esc_html__( 'What is Redux Framework?', 'lenxel-plugin' ), esc_html__( 'Redux Framework', 'lenxel-plugin' ), $this->minimum_capability, 'redux-framework', array( $this, 'about_screen' ) );

			// Support Page.
			$page( esc_html__( 'Get Support', 'lenxel-plugin' ), esc_html__( 'Get Support', 'lenxel-plugin' ), $this->minimum_capability, 'redux-support', array( $this, 'get_support' ) );

			// Status Page.
			$page( esc_html__( 'Redux Health Check', 'lenxel-plugin' ), esc_html__( 'Redux Health Check', 'lenxel-plugin' ), $this->minimum_capability, 'redux-health', array( $this, 'heath_check' ) );
		...
		public function tabs() {
			...
			<h2 class="nav-tab-wrapper">
				<a
					...
					<?php esc_attr_e( 'What is Redux?', 'lenxel-plugin' ); ?>
				</a>
				<a
					...
					<?php esc_attr_e( 'Health Check', 'lenxel-plugin' ); ?>
				</a>
				<a
					...
					<?php esc_attr_e( 'Get Support', 'lenxel-plugin' ); ?>
				</a>
			...
			<p class="redux-actions">
				...
				if ( $options['demo'] ) {
					...
					<a
						...
						class=" button-text button-demo"><?php echo esc_html__( 'Disable Panel Demo', 'lenxel-plugin' ); ?></a>
					<?php
				} else {
					?>
					<a
						href="<?php echo esc_url( admin_url( add_query_arg( $query_args, 'tools.php' ) ) ); ?>"
						class=" button-text button-demo active"><?php echo esc_html__( 'Enable Panel Demo', 'lenxel-plugin' ); ?></a>
					<?php
				}
