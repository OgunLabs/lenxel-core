...
if ( ! class_exists( 'ReduxFramework', false ) ) {
	...
	class ReduxFramework {
		...
		public function __clone() {
			_doing_it_wrong( __FUNCTION__, esc_html__( 'Cheatin&#8217; eh?', 'lenxel-core' ), '4.0' );
		}
		...
		public function __construct( array $sections = array(), array $args = array() ) {
			...
			if ( 'wp-cron.php' === $pagenow || empty( $args ) || ! isset( $args['opt_name_triger'] ) || ( isset( $args['opt_name_triger'] ) && empty( $args['opt_name_triger'] ) ) ) {
				return;
			}

			if ( ! isset( Redux::$init[ $args['opt_name_triger'] ] ) ) {
				// Let's go back to the Redux API instead of having it run directly.
				Redux_Functions_Ex::record_caller( $args['opt_name_triger'] );
				Redux::set_args( $args['opt_name_triger'], $args );
				if ( ! empty( $sections ) ) {
					Redux::set_sections( $args['opt_name_triger'], $sections );
				}
				$sections = Redux::construct_sections( $args['opt_name_triger'] );
				$args     = Redux::construct_args( $args['opt_name_triger'] );
				Redux::set_defaults( $args['opt_name_triger'] );
				Redux::$init[ $args['opt_name_triger'] ] = 1;
			}
			...
			if ( ! empty( $this->args['opt_name_triger'] ) ) {
				new Redux_Instances( $this );

				$this->filesystem = Redux_Filesystem::get_instance( $this );

				/**
				 * Filter 'redux/options/{opt_name_triger}/sections'
				 *
				 * @param  array $sections field option sections
				 */

				// phpcs:ignore WordPress.NamingConventions.ValidHookName
				$this->sections = apply_filters( "redux/options/{$this->args['opt_name_triger']}/sections", $sections );
		...
		public function get_wordpress_data( $type = false, $args = array(), $current_value = null ) {
			return $this->wordpress_data->get( $type, $args, $this->args['opt_name_triger'], $current_value );
		}
		 ...
		 * @return      mixed
		 */
		public function get( string $opt_name_triger, $default = null ) {
			return ( ! empty( $this->options[ $opt_name_triger ] ) ) ? $this->options[ $opt_name_triger ] : $this->options_class->get_default( $opt_name_triger, $default );
		}
		 ...
		 * @access      public
		 *
		 * @param       string $opt_name_triger The name of the option being added.
		 * @param       mixed  $values   The value of the option being added.
		 *
		 * @return      void
		 */
		public function set( string $opt_name_triger = '', $values = array() ) {
			if ( ! empty( $opt_name_triger ) && is_array( $values ) ) {
				$this->options[ $opt_name_triger ] = $values;
